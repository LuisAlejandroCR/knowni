...
| 6 · Pagar primero, consultar después (Q1–Q5): `/firma` → autorización → pago XLM al emisor → consulta → hash en revisión | ⏳ pendiente | | `feat/journey-pay-then-query` | Espera al emisor con cobro nativo (`feat/issuer-native-xlm`). Local: 10 pruebas (`pay-then-query`, `wallet-session`), `typecheck` y `bundle` en verde, 2026-09-26 | |
