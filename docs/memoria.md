...
| 2026-09-24 | Se va el adaptador de Chroma: 110 líneas que solo llamaba su propia prueba, escritas cuando «croma» se leyó como la base vectorial. Lo que no se va es el puerto ni el índice en memoria, y la deuda decía que sobraban los tres: los sostienen `listas.ts` y el recorrido de `journey/`, así que retirarlos es mover el tamizado de listas al camino de Croma, que es trabajo de B3 y no un efecto colateral. La deuda queda con lo que de verdad falta |
| 2026-09-24 | B3 cerrado: el tamizado de listas por nombre se va entero —`listas.ts`, el puerto, el índice en memoria— y el recorrido de `journey/` pasa por `createSanctionsSource`, la fuente que producción ya usaba, con un Croma sintético que responde los tres registros sin red. La prueba del sujeto listado deja de marcarlo por su nombre y lo marca por el registro que lo nombra, que es como responde la fuente real. `sources/` deja de depender de `retrieval/`. Sobreviven `normalize.ts` y `resolve.ts`, hoy sin llamador, porque dos endpoints de Croma consultan por nombre y la regla del margen no se reescribe bien con prisa — D-70. 454 pruebas |
