...
|---|---|---|---|
| 0.0–4.8 | Introducción: tres láminas | — | `intro` |
...
| 16.3–17.9 | Documento completo, botón "Conectar wallet para pagar" | documento | `conectar` desde 16.7, campo difuminado |
| 18.0–18.7 | Wallet: "Entra con tu correo", campo vacío | — | `correo` |
| **18.7–36.0** | **Se escribe el correo; el teclado agranda cada letra** | **correo** | **cortado** |
| 36.2–48.4 | Enviando, "Lo enviamos a <correo>", código `751047` | correo, código | `codigo` ×3, línea y campo difuminados |
...
1. **Guion §1** (`guion.md`): reescribirlo en el orden de esta edición — solicitud → autorización
   → pagar primero → wallet → consulta → Stellar → lo que recibirán → compartir → verificación y
   reuso rechazado → historial — con la columna *Qué se dice* por segmento y su respaldo.
2. **Narración**: una línea por segmento de `SEGMENTS`, que quepa en su duración a ~15 caracteres
   por segundo; si no cabe, alargar el segmento (bajar `speed`) en vez de apretar la voz.
3. **Render local y revisión** (solo en la máquina del titular):
   `python design/demo/build_demo.py`, luego extraer un cuadro cada 0.25 s de `codigo`, `conectar`
   y `pagar` y confirmar que no se lee nada sensible.
4. Mezclar la voz con `ffmpeg` sobre el render, subtítulos quemados en español, y registrar el
   resultado en `guion.md` (entregables).
