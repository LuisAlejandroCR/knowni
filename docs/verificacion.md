...
| El emisor cobra XLM nativo en testnet: con `KNOWNI_PAYMENT_ASSET=native`, `/quote` publica `currency: "XLM"` y términos `native` de 24000000 stroops; `payQuote` con una llave del dispositivo paga, y `verifyPayment` del emisor lo acepta contra Horizon. Sobre la misma transacción, un segundo canje da `already_spent` y exigirla como USDC da `wrong_asset` — criterios X1 y X3 | Testnet de Stellar. Dos cuentas nuevas con friendbot, emisor arrancado con `issuer/src/main.ts`. Explorer: <https://stellar.expert/explorer/testnet/tx/6a26dde9956058d024aa6e22e58739474c96d4d954247decdf08345c53b8c479> · pruebas: `issuer/test/unit/quote-asset.spec.ts` | 2026-09-26 |
...
| El recorrido corre en un iPhone físico contra el emisor real: Registraduría vía Croma, respuesta firmada por `co-operador-demo` (llave fija `ccd01955…`), y el verificador del mismo teléfono la acepta —"Respuestas verificadas"— juzgándola en el momento de compartir | Build de desarrollo EAS `12aeaef3…` + Metro; emisor en `https://issuer.voltarut.com` (túnel Cloudflare `knowni-issuer`); captura del verificador | 2026-09-25 | **Verificado en dispositivo.** La contraparte y el acuse siguen simulados: el verificador corre en el mismo teléfono. Modo avión (A12) sin probar |
...
| 6 · Pagar primero, consultar después (Q1–Q5): `/firma` → autorización → pago XLM al emisor → consulta → hash en revisión | 2026-09-26 | iPhone, build EAS `development` | `feat/journey-pay-then-query` `32df683`; emisor con `KNOWNI_PAYMENT_ASSET=native` | ✅ la autorización pidió conectar la wallet, cotizó 1.2 XLM por una fuente, firmó con Cavos y el emisor consultó solo después de verificar el pago | tx `3c1bdad580f475f8c0608246bf1508d9e319b39efa57c0006f00f16cc07a01d8`, ledger 4876624, 07:11:47Z; Horizon: `successful: true`, `payment` nativo de 1.2000000 de `GDCI23MF…VWWT` a la tesorería `GD4XN7C5…MP7VS3`, memo hash `bccf74e7…712f`; el emisor anotó el hash en su conjunto de pagos gastados. Toma grabada del video demo: tx `53e6ea60810e6c585c48a45d4fa095aa73e6bdaffdb3be5f7cec71f3ddd52724`, ledger 4876770, 07:23:57Z; Horizon (releído 2026-09-26): `successful: true`, `payment` nativo de 1.2000000 de `GDCI23MF…VWWT` a `GD4XN7C5…MP7VS3`, con `MEMO_HASH` |
...
| "ninguna llamada sobre una persona real" | Ningún fixture ni bitácora contiene una consulta con documento real | ⚠️ **desactualizada desde 2026-09-25**: el titular consultó su propia Registraduría desde el iPhone, con consentimiento en pantalla. El documento no quedó en el repositorio ni en bitácora |
