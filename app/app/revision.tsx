    ...
    <Screen>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24 }}>
        ...
        {paymentTx === undefined ? null : (
          <>
            <Label>Pago de esta consulta</Label>
            <Card>
              <Row
                icon="◎"
                title={`${paymentTx.slice(0, 8)}…${paymentTx.slice(-8)}`}
                scope="Transacción en Stellar testnet · ver en Stellar Expert"
                trailing="↗"
                onPress={() => void Linking.openURL(explorerUrl(paymentTx))}
              />
            </Card>
          </>
        )}
