...
| 6 · Recorrido principal paga y luego consulta (J1–J6, A10) | pendiente | iPhone, build EAS `development` | `feat/journey-pay-then-query` | ⏳ `/firma` → consentimiento «Pagar X XLM y consultar» → emisión con hash → revisión con enlace. Requiere el emisor con cobro en XLM nativo (`feat/issuer-native-xlm`) | Anotar el hash y `curl https://horizon-testnet.stellar.org/transactions/<hash>`: `successful: true`, `memo_type: hash` = `paymentRef` de la cotización, `payment` nativo al destino del emisor |
