...
| 5c · El recorrido paga XLM nativo y después consulta (J1–J6) | | iPhone, build EAS `development` | `feat/journey-pay-then-query` | ⏳ pendiente: entrar en `/firma` → consentimiento dice «Pagar X XLM y consultar» → emisión muestra el hash → revisión lo enlaza | hash de la transacción y `curl` a Horizon con `successful: true`, `memo` = `paymentRef` y destino la tesorería del emisor |
