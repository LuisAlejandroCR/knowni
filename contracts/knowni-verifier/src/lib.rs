...
fn signals_match(env: &Env, named: &PublicSignals, raw: &Vec<Fr>) -> bool {
    ...
    fr_eq_bool(env, raw.get(0), named.personhood)
        && fr_eq_u32(env, raw.get(1), named.solvency_tier)
        && fr_eq_bool(env, raw.get(2), named.formality)
        && fr_eq_bool(env, raw.get(3), named.standing)
        && fr_eq_bytes(raw.get(4), &named.nullifier)
        && fr_eq_bytes(raw.get(5), &named.issuer_root)
        && fr_eq_bytes(raw.get(6), &named.session_id)
        && fr_eq_bytes(raw.get(11), &named.list_set_root)
}
