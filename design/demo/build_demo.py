...
# panels, a live zoom card and a journey progress bar. Outputs out/knowni-demo.mp4.
...
SEGMENTS = [
    dict(name="intro", a=0.2, b=4.6, step=-1, eyebrow="KNOWNI · DEMO EN UN IPHONE REAL",
         title="Demuestra más.\nRevela menos.",
         sub="Respondes lo que te piden para firmar un contrato,\nsin entregar tu cédula ni tu expediente."),
    dict(name="solicitud", a=5.0, b=8.0, step=0, eyebrow="01 · SOLICITUD",
    ...
    dict(name="autoriza", a=8.0, b=11.9, speed=1.3, step=1, eyebrow="02 · AUTORIZACIÓN",
         title="Consultar\nno es compartir.",
    ...
    dict(name="conectar", a=16.7, b=17.9, step=2, eyebrow="03 · PAGO PRIMERO",
         title="Sin pago,\nno hay consulta.",
    ...
    dict(name="correo", a=18.0, b=18.7, step=2, blurs=[(EMAIL_FIELD, 0, 1)], eyebrow="04 · WALLET CAVOS",
         title="Tu wallet vive\nen el teléfono.",
         sub="Entras con un código por correo. Sin contraseñas\ny sin semilla que anotar."),
    dict(name="codigo", a=36.2, b=48.4, speed=3.0, step=2, eyebrow="04 · WALLET CAVOS",
         title="Tu wallet vive\nen el teléfono.",
    ...
    dict(name="espacio", a=108.0, b=111.0, step=6, eyebrow="11 · MI ESPACIO",
         title="Todo queda\nen tu teléfono.",
         sub="Historial de lo que compartiste y rechazaste.\nTu documento nunca llega a la contraparte."),
]

TITLE_CARD = dict(eyebrow="STELLAR TESTNET · 26/09/2026", title="knowni",
                  sub="Una verificación real, grabada en un iPhone.\nDatos personales difuminados.", dur=3.0)
END_CARD = dict(eyebrow="LO QUE ACABAS DE VER", title="Demuestra más.\nRevela menos.",
...
def segment(i, seg, bezel_png):
    ...
    script = os.path.join(WORK, f"seg_{i:02d}.txt")
    with open(script, "w", encoding="utf-8") as fh:
        fh.write(";\n".join(f))
    run(["ffmpeg", "-v", "error", "-y", "-ss", str(seg["a"]), "-t", str(seg["b"] - seg["a"]), "-i", SRC,
         "-loop", "1", "-i", bg, "-loop", "1", "-i", bezel_png,
         "-f", "lavfi", "-i", "anullsrc=r=48000:cl=stereo",
         "-filter_complex_script", script, "-map", "[out]", "-map", "3:a", "-t", str(dur),
...
def card(i, spec):
    ...
    out = os.path.join(WORK, f"seg_{i:02d}.mp4")
    d = spec["dur"]
    run(["ffmpeg", "-v", "error", "-y", "-loop", "1", "-i", bg, "-f", "lavfi", "-i", "anullsrc=r=48000:cl=stereo",
         "-vf", f"fade=t=in:st=0:d=0.3:color=0x193e36,fade=t=out:st={d - 0.3}:d=0.3:color=0x193e36,format=yuv420p",
         "-t", str(d), "-c:v", "libx264", "-preset", "medium", "-crf", "18", "-r", "30",
...
def main():
    ...
    for (p, d), seg in zip(parts, [TITLE_CARD] + SEGMENTS + [END_CARD]):
        timeline.append(dict(start=round(t, 2), dur=d, name=seg.get("name", seg["eyebrow"]), title=seg["title"]))
        t += d
