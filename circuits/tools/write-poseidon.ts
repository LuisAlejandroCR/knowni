...
function template(inputs: number): string {
  ...
  lines.push(`                fullSquared[f][i] <== (state[r][i] + C[r * ${t} + i]) * (state[r][i] + C[r * ${t} + i]);`);
  lines.push("                fullQuartic[f][i] <== fullSquared[f][i] * fullSquared[f][i];");
  lines.push(`                fullSboxed[f][i] <== fullQuartic[f][i] * (state[r][i] + C[r * ${t} + i]);`);
  lines.push("            }");
  lines.push(`            for (var i = 0; i < ${t}; i++) {`);
  lines.push("                var lc = 0;");
  lines.push(`                for (var j = 0; j < ${t}; j++) {`);
  lines.push("                    lc += M[i][j] * fullSboxed[f][j];");
  lines.push("                }");
  ...
  lines.push("            // The other cells stay linear, so they enter the mix as the");
  lines.push("            // expression they already are instead of a signal each.");
  lines.push(`            partialSquared[p] <== (state[r][0] + C[r * ${t}]) * (state[r][0] + C[r * ${t}]);`);
  lines.push("            partialQuartic[p] <== partialSquared[p] * partialSquared[p];");
  lines.push(`            partialSboxed[p] <== partialQuartic[p] * (state[r][0] + C[r * ${t}]);`);
  lines.push(`            for (var i = 0; i < ${t}; i++) {`);
  lines.push("                var lc = M[i][0] * partialSboxed[p];");
  lines.push(`                for (var j = 1; j < ${t}; j++) {`);
  lines.push(`                    lc += M[i][j] * (state[r][j] + C[r * ${t} + j]);`);
  lines.push("                }");
  lines.push("                state[r + 1][i] <== lc;");
  lines.push("            }");
  lines.push("            p++;");
